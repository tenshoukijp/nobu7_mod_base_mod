...
void WriteAsmJumperOnNb7FontnameAddressPush() {
	...
	BOOL ret = WriteProcessMemory(hCurrentProcess, (LPVOID)(pNB7JumpFromToOnNB7FontnameAddressPush), cmdOBN7JumpFromNB7FontnameAddressPush, 5, NULL); //5バイトのみ書き込む
	if (ret) {
		OutputDebugString("書き換え成功\n");
	}
	else {
		OutputDebugString("書き換え失敗\n");
	}
}
