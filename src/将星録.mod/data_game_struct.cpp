...
void setGameDataStructPointer() {
	...
	for (int iBushouID = 0; iBushouID < 最大数::武将情報::配列数; iBushouID++) {
		...
		if (getBushou苗字FromBushouID(iBushouID) == "宰相") {
			...
			OutputDebugStream("unknown11:%d ", (int)(nb7武将情報[iBushouID].unknown11));
			OutputDebugStream("unknown11:%d ", (int)(nb7武将情報[iBushouID].unused7));
			OutputDebugStream("忍者修行:%d ", nb7武将情報[iBushouID].忍者習得済);
			OutputDebugStream("行動済み:%d ", nb7武将情報[iBushouID].行動済);
			nb7武将情報[iBushouID].移動力増加済 = 1;
			OutputDebugStream("\n");

			for (int iKahouID = 0; iKahouID < 最大数::家宝情報::配列数; iKahouID++) {
				if (nb7家宝情報[iKahouID].家宝名 == "青磁千鳥"s) {
					setBushouIDToKahouID(iKahouID, iBushouID);
					OutputDebugStream("所有者を変更しました\n");
				}
			}
