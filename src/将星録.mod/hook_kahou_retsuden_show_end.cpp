...
static int iKahouShowEndEBP = -1;
static int iKahouShowEndECX = -1; // 対象家宝へのポインタ。家宝配列への特定のポインタ
void OnSSRExeKahouShowEndExecute() {
	isKahouRetsudenBeginning = false;

	OutputDebugStream("★★★★家宝表示終了!!!★\n");
	OutputDebugStream("★★★★EBP:%x!!!★\n", iKahouShowEndEBP);
	OutputDebugStream("★★★★ECX:%x!!!★\n", iKahouShowEndECX);
	int iKahouID = (iKahouShowEndECX - 家宝情報アドレス) / sizeof(NB7家宝情報型);
...
0047E9C5   . 50             PUSH EAX
*/
int pSSRExeJumpFromToOnSSRExeKahouShowEnd = 0x47E9C0; // 関数はこのアドレスから、OnSSRExeKahouShowEndへとジャンプしてくる。
int pSSRExeJumpCallFromToOnSSRExeKahouShowEnd = 0x47C070; // 元々あった処理のCall先
int pSSRExeReturnLblFromOnSSRExeKahouShowEnd = 0x47E9C5; // 関数が最後までいくと、このTENSHOU.EXE内に直接ジャンプする

#pragma warning(disable:4733)

__declspec(naked) void WINAPI OnSSRExeKahouShowEnd() {
	// スタックにためておく
	__asm {
		mov iKahouShowEndECX, ecx
		mov iKahouShowEndEBP, ebp
		push eax
		push ebx
		push ecx
		push edx
	...
	__asm {
		...
		call pSSRExeJumpCallFromToOnSSRExeKahouShowEnd // 元の処理

		jmp pSSRExeReturnLblFromOnSSRExeKahouShowEnd
	}
}
...
char cmdOnSSRExeJumpFromKahouShowEnd[6] = "\xE9";
// 元の命令が5バイト、以後の関数で生まれる命令が合計５バイトなので… 最後１つ使わない


// ニーモニック書き換え用
void WriteAsmJumperOnSSRExeKahouShowEnd() {

	// まずアドレスを数字として扱う
	int iAddress = (int)OnSSRExeKahouShowEnd;
	int SubAddress = iAddress - (pSSRExeJumpFromToOnSSRExeKahouShowEnd + 5);
	// ５というのは、0046C194  -E9 ????????  JMP TSMod.OnTSExeGetDaimyoKoukeishaBushouID  の命令に必要なバイト数。要するに５バイト足すと次のニーモニック命令群に移動するのだ。そしてそこからの差分がジャンプする際の目的格として利用される。
	memcpy(cmdOnSSRExeJumpFromKahouShowEnd + 1, &SubAddress, 4); // +1 はE9の次から4バイト分書き換えるから。

	// 構築したニーモニック命令をTENSHOU.EXEのメモリに書き換える
	WriteProcessMemory(hCurrentProcess, (LPVOID)(pSSRExeJumpFromToOnSSRExeKahouShowEnd), cmdOnSSRExeJumpFromKahouShowEnd, 5, NULL); //5バイトのみ書き込む
}
