...
BOOL nPlayBGM3NextIsMMio = FALSE; // この直後のMMIOによる制御が本当に将星録のものであることの保証に利用する
...
void OnSSRExePlayBGM3Execute() {
	nPlayBGM3NextIsMMio = TRUE; // このフラグは今のところ利用していないが、今後のために残しておく。
...
int pSSRExeReturnLblFromOnSSRExePlayBGM3 = 0x419306; // 関数が最後までいくと、この実行exe内に直接ジャンプする
