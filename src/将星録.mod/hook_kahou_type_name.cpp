...
void OverrideKahouTypeName(int iKahouID) {
	...
	try {
		...
		System::Collections::Generic::Dictionary<System::String^, System::Object^>^ ret = InvokeUserMethod("on‰Æ•óí—Ş–¼—v‹", dic);

		if (ret != nullptr && ret->ContainsKey("í—Ş–¼")) {
			System::String^ kahou_type_name = (System::String^)(ret["í—Ş–¼"]);
			if (System::String::IsNullOrEmpty(kahou_type_name)) {
				;
			}
			else {
				string native_typename = to_native_string(kahou_type_name);
				native_typename[8] = '\0'; // 8•¶š‚Ü‚Å
		...
		OutputDebugStream("on‰Æ•óí—Ş–¼—v‹‚Å‚ÌƒGƒ‰[");
	}
...
int pSSRExeReturnLblFromOnSSRExeKahouTypeName = 0x47CA31; // ŠÖ”‚ªÅŒã‚Ü‚Å‚¢‚­‚ÆA‚±‚ÌÀsexe“à‚É’¼ÚƒWƒƒƒ“ƒv‚·‚é
