...
int iLastCastleBattleDaimyoB_ID = -1;
...
void OnSSRExeCastleBattleTurnDaimyoBExecute() {
	...
	iLastCastleBattleDaimyoB_ID = iDaimyoID;
...
0040E6A9   8B38             MOV EDI,DWORD PTR DS:[EAX]
0040E6AB   B9 280F5400      MOV ECX,Nobunaga.00540F28
0040E6B0   E8 AB92FFFF      CALL Nobunaga.00407960 // 攻撃側の大名ポインタを得る
0040E6B5   50               PUSH EAX
0040E6B6   8BCF             MOV ECX,EDI
0040E6B8   E8 A3950300      CALL Nobunaga.00447C60
0040E6BD   50               PUSH EAX
...
__declspec(naked) void WINAPI OnSSRExeCastleBattleTurnDaimyoB() {
	// スタックにためておく
	__asm {
		mov iCastleBattleDaimyoPointer, EDI
		mov iCastleBattleECX, ECX

		call pSSRExeJumpCallFromToOnSSRExeCastleBattleTurnDaimyoB // 元の処理
	...
	OnSSRExeCastleBattleTurnDaimyoBExecute();
	...
	__asm {
		...
		jmp pSSRExeReturnLblFromOnSSRExeCastleBattleTurnDaimyoB
	}
}
...
char cmdOnSSRExeJumpFromCastleBattleTurnDaimyoB[6] = "\xE9";
// 元の命令が5バイト、以後の関数で生まれる命令が合計５バイトなので… 最後１つ使わない


// ニーモニック書き換え用
void WriteAsmJumperOnSSRExeCastleBattleTurnDaimyoB() {

	// まずアドレスを数字として扱う
	int iAddress = (int)OnSSRExeCastleBattleTurnDaimyoB;
	int SubAddress = iAddress - (pSSRExeJumpFromToOnSSRExeCastleBattleTurnDaimyoB + 5);
	// ５というのは、0046C194  -E9 ????????  JMP TSMod.OnTSExeGetDaimyoKoukeishaBushouID  の命令に必要なバイト数。要するに５バイト足すと次のニーモニック命令群に移動するのだ。そしてそこからの差分がジャンプする際の目的格として利用される。
	memcpy(cmdOnSSRExeJumpFromCastleBattleTurnDaimyoB + 1, &SubAddress, 4); // +1 はE9の次から4バイト分書き換えるから。

	// 構築したニーモニック命令をTENSHOU.EXEのメモリに書き換える
	WriteProcessMemory(hCurrentProcess, (LPVOID)(pSSRExeJumpFromToOnSSRExeCastleBattleTurnDaimyoB), cmdOnSSRExeJumpFromCastleBattleTurnDaimyoB, 5, NULL); //5バイトのみ書き込む
}
