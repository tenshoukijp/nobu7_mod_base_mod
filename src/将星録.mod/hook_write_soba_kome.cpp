...
void OnSSRExeWriteSobaKomeExecute() {
	OutputDebugStream("★★★★米相場が初書き込みされました!!!:%d\n", iWriteSobaKomeEAX);
...
__declspec(naked) void WINAPI OnSSRExeWriteSobaKome() {
	...
	__asm {
		...
		call pSSRExeJumpCallFromToOnSSRExeWriteSobaKome // 元の処理
...
void WriteAsmJumperOnSSRExeWriteSobaKome() {
	...
	memcpy(cmdOnSSRExeJumpFromWriteSobaKome + 1, &SubAddress, 4); // +1 はE9の次から4バイト分書き換えるから。

	// 構築したニーモニック命令をTENSHOU.EXEのメモリに書き換える
	WriteProcessMemory(hCurrentProcess, (LPVOID)(pSSRExeJumpFromToOnSSRExeWriteSobaKome), cmdOnSSRExeJumpFromWriteSobaKome, 5, NULL); //5バイトのみ書き込む
}
