...
0047E757   . E8 04DCFFFF    CALL Nobunaga.0047C360 　　　　　　// ★この関数が家宝の画像を表示している
...
/*
家宝を表示している関数
0047C360   85C9             TEST ECX,ECX
0047C362  ^74 90            JE SHORT Nobunaga.0047C2F4
0047C364   833D 88535800 00 CMP DWORD PTR DS:[585388],0
0047C36B  ^75 90            JNZ SHORT Nobunaga.0047C2FD
0047C36D   8A01             MOV AL,BYTE PTR DS:[ECX]
0047C36F   84C0             TEST AL,AL
0047C371  ^74 90            JE SHORT Nobunaga.0047C303
0047C373   8B49 16          MOV ECX,DWORD PTR DS:[ECX+16]
0047C376   83F9 1A          CMP ECX,1A                             // ★家宝画像の26種類という制限
0047C379   77 08            JA SHORT Nobunaga.0047C383
0047C37B   8D81 370A0000    LEA EAX,DWORD PTR DS:[ECX+A37]
0047C381   EB 03            JMP SHORT Nobunaga.0047C386
0047C383   83C8 FF          OR EAX,FFFFFFFF
0047C386   8B4C24 08        MOV ECX,DWORD PTR SS:[ESP+8]
0047C38A   8B5424 04        MOV EDX,DWORD PTR SS:[ESP+4]
0047C38E   6A 10            PUSH 10
0047C390   6A 17            PUSH 17
0047C392   6A 00            PUSH 0
0047C394   51               PUSH ECX
0047C395   52               PUSH EDX
0047C396   50               PUSH EAX
0047C397   B9 380F5800      MOV ECX,Nobunaga.00580F38
0047C39C   E8 DF91FEFF      CALL Nobunaga.00465580
0047C3A1   C2 0800          RETN 8
...
void OnSSRExeKahouShowPicExecute() {
	OutputDebugStream("KahouShowPic\n");
}
...
__declspec(naked) void WINAPI OnSSRExeKahouShowPic() {
	// スタックにためておく
	__asm {
	...
	__asm {
		...
		call pSSRExeJumpCallFromToOnSSRExeKahouShowPic // 元の処理
	...
	__asm {
		...
		jmp pSSRExeReturnLblFromOnSSRExeKahouShowPic
	}
}
...
char cmdOnSSRExeJumpFromKahouShowPic[6] = "\xE9";
// 元の命令が5バイト、以後の関数で生まれる命令が合計５バイトなので… 最後１つ使わない


// ニーモニック書き換え用
void WriteAsmJumperOnSSRExeKahouShowPic() {

	// まずアドレスを数字として扱う
	int iAddress = (int)OnSSRExeKahouShowPic;
	int SubAddress = iAddress - (pSSRExeJumpFromToOnSSRExeKahouShowPic + 5);
