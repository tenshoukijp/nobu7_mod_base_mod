...
void OnSSRExeUnknownDialog1Execute() {
	OutputDebugStream("★★★★不明なダイアログ1が来たよ!!!★\n");
}
...
*/
int pSSRExeJumpFromToOnSSRExeUnknownDialog1 = 0x42FE24; // 関数はこのアドレスから、OnSSRExeUnknownDialog1へとジャンプしてくる。
int pSSRExeJumpCallFromToOnSSRExeUnknownDialog1 = 0x042C970; // 元々あった処理のCall先
int pSSRExeReturnLblFromOnSSRExeUnknownDialog1 = 0x42FE29; // 関数が最後までいくと、このTENSHOU.EXE内に直接ジャンプする

#pragma warning(disable:4733)

__declspec(naked) void WINAPI OnSSRExeUnknownDialog1() {
	// スタックにためておく
	__asm {
	...
	__asm {
		...
		call pSSRExeJumpCallFromToOnSSRExeUnknownDialog1 // 元の処理

		jmp pSSRExeReturnLblFromOnSSRExeUnknownDialog1
	}
}
...
char cmdOnSSRExeJumpFromUnknownDialog1[6] = "\xE9";
// 元の命令が5バイト、以後の関数で生まれる命令が合計５バイトなので… 最後１つ使わない


// ニーモニック書き換え用
void WriteAsmJumperOnSSRExeUnknownDialog1() {

	// まずアドレスを数字として扱う
	int iAddress = (int)OnSSRExeUnknownDialog1;
	int SubAddress = iAddress - (pSSRExeJumpFromToOnSSRExeUnknownDialog1 + 5);
	// ５というのは、0046C194  -E9 ????????  JMP TSMod.OnTSExeGetDaimyoKoukeishaBushouID  の命令に必要なバイト数。要するに５バイト足すと次のニーモニック命令群に移動するのだ。そしてそこからの差分がジャンプする際の目的格として利用される。
	memcpy(cmdOnSSRExeJumpFromUnknownDialog1 + 1, &SubAddress, 4); // +1 はE9の次から4バイト分書き換えるから。

	// 構築したニーモニック命令をTENSHOU.EXEのメモリに書き換える
	WriteProcessMemory(hCurrentProcess, (LPVOID)(pSSRExeJumpFromToOnSSRExeUnknownDialog1), cmdOnSSRExeJumpFromUnknownDialog1, 5, NULL); //5バイトのみ書き込む
}
