...
void doGameTest() {
	...
	for (int i = 0; i < 最大数::大名情報::配列数; i++) {
		...
		if (isValidBushouID(iBushouID)) {
			OutputDebugStream("%s旗番号%d\n", nb7武将情報[iBushouID].姓名, nb7大名情報[i].旗番号);
		}
		else {
			OutputDebugStream("%s旗番号%d\n", "大名なし", nb7大名情報[i].旗番号);
		}
	...
	for (int iCastleID = 0; iCastleID < 最大数::城情報::配列数; iCastleID++) {
		if (nb7城情報[iCastleID].城名 == "浜松"s) {
			OutputDebugStream("浜松の委任:%d\n", (int)nb7城情報[iCastleID].委任状態);
			OutputDebugStream("浜松の委任攻撃:%d\n", (int)nb7城情報[iCastleID].委任攻撃);
			OutputDebugStream("浜松の委任？:%d\n", (int)nb7城情報[iCastleID].unused1);
		}
	}
	*/
	...
	*/
	...
	for (int iCastleID = 0; iCastleID < 最大数::城情報::配列数; iCastleID++) {
		...
		OutputDebugStream(get城称(iCastleID) + "\n");
	}
	*/
	...
	*/
	...
	for (int iDaimyouID = 0; iDaimyouID < 最大数::大名情報::配列数; iDaimyouID++) {
		...
		if (nb7大名情報[iDaimyouID].unknown17) {
			OutputDebugStream("番号:%d\t", iDaimyouID);
			OutputDebugStream("大名:%s\n", nb7大名情報[iDaimyouID].p大名武将);
			OutputDebugStream("unknown17:%d\n", nb7大名情報[iDaimyouID].unknown17);
		}
		if (nb7大名情報[iDaimyouID].unknown18) {
			OutputDebugStream("番号:%d\t", iDaimyouID);
			OutputDebugStream("大名:%s\n", nb7大名情報[iDaimyouID].p大名武将);
			OutputDebugStream("unknown18:%d\n", nb7大名情報[iDaimyouID].unknown18);
		}
	}
	*/
}
