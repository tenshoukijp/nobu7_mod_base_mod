...
std::vector<NB7籠城戦防御側部隊情報型> list防御側部隊情報;
...
void OnSSRExeCastleBattleDefendHeisuuSumExecute() {
	...
	if (index == 0) {
		list防御側部隊情報.clear();
		for (int i = 0; i < 最大数::籠城戦::防御部隊情報::配列数; i++) { // 0x16=22
			NB7籠城戦防御側部隊情報型 info;
	...
	if (index < (int)list防御側部隊情報.size()) {
		NB7籠城戦防御側部隊情報型* p部隊情報 = (NB7籠城戦防御側部隊情報型*)ECXOfCastleBattleDefendHeisuuSum;
...
int pSSRExeReturnLblFromOnSSRExeCastleBattleDefendHeisuuSum = 0x4076CD; // 関数が最後までいくと、この実行exe内に直接ジャンプする
