...
extern std::map<string, string> Š¿šToŠOš;
extern std::map<string, string> ŠOšToŠ¿š;
...
void •«î•ñŒ^::g•ª::set(int value) {
	if (•«::g•ª::‘«Œy“ª <= value && value < •«::g•ª::‘å–¼) {
		...
		throw gcnew System::ArgumentOutOfRangeException("g•ª‚ª•s³‚Å‚·B");
	}
}
