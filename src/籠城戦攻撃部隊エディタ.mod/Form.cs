...
public partial class 籠城戦攻撃部隊エディタ : 基本エディタ
{

    public 籠城戦攻撃部隊エディタ() : base()
    {
        this.Text = nameof(籠城戦攻撃部隊エディタ);
    }

    class BIND用の籠城戦攻撃部隊情報型
    {
        籠城戦.攻撃部隊情報型 籠城戦攻撃部隊情報;
        public BIND用の籠城戦攻撃部隊情報型(int 部隊配列IX)
        {
            籠城戦攻撃部隊情報 = new 籠城戦.攻撃部隊情報型(部隊配列IX);
        }

        public int 配列IX
        {
            get { return 籠城戦攻撃部隊情報.配列IX; }
        }
        ...
        public string 指揮武将姓名
        {
            get
            {
                int iBushouID = 籠城戦攻撃部隊情報.指揮武将配列IX;
        ...
        public int 指揮武将配列IX
        {
            get { return 籠城戦攻撃部隊情報.指揮武将配列IX; }
        }
        public int 部隊番号
        {
            get { return 籠城戦攻撃部隊情報.部隊番号; }
        }

        public int 兵数
        {
            get { return 籠城戦攻撃部隊情報.兵数; }
            set { 籠城戦攻撃部隊情報.兵数 = value; }
        }
    ...
    protected override void dvg_DataBinding()
    {
        try
        {
            List<BIND用の籠城戦攻撃部隊情報型> データ配列 = new();
            for (int i = 0; i < 将星録.最大数.籠城戦.攻撃部隊情報.配列数; i++)
            {
                データ配列.Add(new BIND用の籠城戦攻撃部隊情報型(i));
            }
    ...
    protected override void dvg_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
    {
        try
        {
            dgv.Columns[nameof(BIND用の籠城戦攻撃部隊情報型.配列IX)].DefaultCellStyle.BackColor = Color.LightGray;
